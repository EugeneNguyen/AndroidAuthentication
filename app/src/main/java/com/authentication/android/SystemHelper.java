...
public class SystemHelper {
    ...
    {
        try {
            PackageInfo info = v.getPackageManager().getPackageInfo(v.getPackageName(), PackageManager.GET_SIGNATURES);
            for (Signature signature : info.signatures) {
                MessageDigest md = MessageDigest.getInstance("SHA");
                md.update(signature.toByteArray());
                String myhash = Base64.encodeToString(md.digest(), Base64.DEFAULT);
                Log.e("TEMPTAGHASH KEY:", myhash);
            }
...
//    }
    ...
    public static void showDialog(Context v, String title, String message)
    {
        if(pd == null)
    ...
    {
        v.getLayoutParams().height = (int) (SystemHelper.getHeight(context) * h);
        v.getLayoutParams().width = (int) (SystemHelper.getWidth(context) * w);
    }
